...
export class ParticleStorm implements VisualizerScene {
    ...
    // Particle data (SoA layout for performance)
    ...
    // Tracking: last noteOn note to detect new bursts
    private lastProcessedEvent: number = 0;
    ...
    init(canvas: HTMLCanvasElement, width: number, height: number) {
        ...
        // Initialise all particles as dead (age > lifetime)
    ...
    update(resolved: ResolvedParams, state: MidiState, dt: number) {
        ...
        // Detect new noteOn events and spawn burst
        if (
            state.lastEvent?.type === "noteon" &&
            state.lastEvent.velocity > 0
        ) {
            const eventId = state.lastEvent.note * 1000 + state.lastEvent.velocity;
            if (eventId !== this.lastProcessedEvent) {
                this.lastProcessedEvent = eventId;
                this.spawnBurst(hue, sizeScale, speed, spread, state.lastEvent.velocity / 127);
            }
        }
        ...
        for (let i = 0; i < MAX_PARTICLES; i++) {
            if (this.ages[i] >= this.lifetimes[i]) continue; // dead particle

            this.ages[i] += dt;
            const t = this.ages[i] / this.lifetimes[i]; // 0→1 over lifetime
            if (t >= 1) {
                this.sizes[i] = 0;
                continue;
            }
            ...
            // Fade out alpha by darkening color
            const fade = 1 - t * t; // ease-out fade
            this.colors[i3] *= 0.99;
    ...
    private spawnBurst(
        ...
        for (let b = 0; b < count; b++) {
            ...
            this.velocities[i3 + 1] = r * Math.sin(phi) * Math.sin(theta) * spread * 6 + 3;
            this.velocities[i3 + 2] = r * Math.cos(phi) * spread * 4;

            // Color with slight per-particle variation
