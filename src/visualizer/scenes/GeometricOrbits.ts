...
export class GeometricOrbits implements VisualizerScene {
    ...
    update(resolved: ResolvedParams, state: MidiState, dt: number) {
        ...
        for (const note of currentNotes) {
            if (!this.meshes.has(note)) {
                this.spawnMesh(note, state.activeNotes.get(note)!.velocity / 127, sizeScale);
            }
        }
    ...
    private releaseMesh(note: number) {
        ...
        orb.tween?.kill();
        // Animate fade/shrink then remove
